<?php
	 ...
	 */
	get_header();
	$detail_dialogue=get_detail_dialogue();
?>
<div class="content-wrapper">
	<div class="content main" id="main">
		<div class="letter-list">
			<h6 class="subject">F2E>历史记录>对话</h6>
<?php 
	if(is_array($detail_dialogue)):
		foreach($detail_dialogue as $letter):
?>
			<div class="letter">
	<?php 
		if(isset($letter['from_uid'])):
			$user_id=$letter['from_uid'];
			$username=get_username($user_id);
	?>
			<a href="fe-user.php?u=<?php echo $user_id;?>" class="small-face"><img src="<?php echo fe_userFace($user_id,'small');?>"/></a>
			<div class="letter-content">
				<a href="fe-user.php?u=<?php echo $user_id;?>"><?php echo $username;?></a>发送给我
	<?php 
		else:
			$user_id=$letter['to_uid'];
			$username=get_username($user_id);
			$self_id=get_user_id(null,true);
	?>
			<a href="fe-user.php?u=<?php echo $user_id;?>" class="small-face"><img src="<?php echo fe_userFace($self_id,'small');?>" /></a>
			<div class="letter-content">
				我发送给<a href="fe-user.php?u=<?php echo $user_id;?>"><?php echo get_username($user_id);?></a>:
	<?php
		endif;
	?>
				<div class="letter-text"><?php echo $letter['content'];?></div>
				<div class="meta">
					<span class="time"><?php echo format_time($letter['cTime']);?></span>
					<span class="manipulate">
			<?php
				if(isset($letter['from_uid'])){
					echo "<a href='fe-action.php?act=deliver&u={$user_id}&name={$username}' class='reply'>回复</a>";
				}
			?>
						<a href="fe-action.php?act=delLetter&l=<?php echo $letter['id'];?>" class="delete">删除</a>
					</span>
				</div>
			</div>
			</div>
...
?>
		<p class='no-mention'>您暂时还没有私信对话记录</p>
<?php 
	endif;
?>
		</div>
...
<h3>删除私信<a href="javascript:" id="cancel"></a></h3>
确认删除？
...
<script type="text/javascript">
	...
	function manipulate(event){
		...
		else if(clsName=='delete'){	//点击了删除链接
